...
> **Version History**
> - 0.2.0: Integrated design feedback - triggers as modules, background_sessions as top-level config, auto-emit session:completed, protocol refinements
...
class SpawnResult:
    ...
    # NOTE: events_emitted was removed per design feedback.
    # Events are fire-and-forget; tracking belongs in EventRouter history
    # if needed, not in spawn results.
...
class SessionStorage(Protocol):
    ...
    async def delete(self, session_id: str) -> bool:
        """
        Delete session state. Returns True if deleted, False if not found.
        
        Useful for cleanup of completed background sessions or
    ...
    async def list_sessions(
        ...
        Args:
            ...
            bundle_name: Filter by bundle name
            limit: Maximum number of results
            
        Returns:
            List of session metadata dicts (not full transcripts)
        """
        ...
...
class ContextInheritance(Enum):
    ...
    # NOTE: SUMMARY was removed from this enum per design feedback.
    # Context summarization is a separate concern that involves:
    # - LLM calls (policy about which model, what prompt)
    # - Token budget decisions (how long should summary be)
    # - Caching strategy (summarize once vs every spawn)
    #
    # This should be handled by a separate summarizer capability/module,
    # not embedded in the spawn primitive. Example usage:
    #
    #   summary = await summarizer.summarize(parent_context)
    #   result = await spawn_bundle(
    #       bundle=...,
    #       instruction=f"{summary}\n\n{instruction}",
    #       inherit_context_messages=ContextInheritance.NONE,
    #   )
```
    ...
    if background:
        ...
        task = asyncio.create_task(
            _execute_background_session(
                ...
                session_storage,
                sub_session_id,
                event_router,  # Pass router for auto session:completed emit
            )
        )
        # Return immediately with session ID
        # Callers subscribe to session:completed event for results
        return SpawnResult(
...
> **Design Principle**: Foundation defines the **protocol**; implementations are **pluggable modules**.
> This follows Amplifier's kernel philosophy: "Mechanism, not policy."
...
class TriggerSource(Protocol):
    ...
    Foundation provides this protocol. Implementations live in modules:
    - trigger-file-watcher: File system change detection
    - trigger-timer: Cron and interval-based triggers
    - trigger-issue: Issue event polling/webhooks
    - trigger-session-event: Cross-session event subscription
    
    This separation allows teams to swap implementations (e.g., different
    file watcher backends, different timer implementations) without
    changing foundation.
    """
    ...
    def configure(self, config: dict) -> None:
        """Configure the trigger source from bundle config."""
        ...


class TriggerSourceFactory(Protocol):
    """Factory for creating trigger sources from configuration."""
    
    def create(self, trigger_config: SessionTrigger) -> TriggerSource:
        """Create a trigger source from configuration."""
        ...
```

#### Trigger Module Contract

Trigger modules implement the `TriggerSource` protocol and are loaded like other modules:

```yaml
# Bundle configuration for triggers
triggers:
  - module: trigger-file-watcher
    source: foundation:modules/trigger-file-watcher  # Or external
    config:
      patterns: ["**/*.py", "**/*.ts"]
      debounce_ms: 2000
      
  - module: trigger-timer
    config:
      interval_seconds: 300  # Every 5 minutes
      
  - module: trigger-session-event
    config:
      event_names: ["work:completed", "tests:failed"]
```

#### Reference Implementations (in foundation:modules/)

Foundation provides reference trigger implementations:

| Module | Event Type | Description |
|--------|-----------|-------------|
| `trigger-file-watcher` | FILE_CHANGE | Uses watchdog for file system events |
| `trigger-timer` | TIMER | Cron and interval-based triggers |
| `trigger-issue` | ISSUE_EVENT | Polls issue store or receives webhooks |
| `trigger-session-event` | SESSION_EVENT | Subscribes to EventRouter |

Teams can provide alternative implementations (e.g., `trigger-inotify` for Linux-specific watching, `trigger-cloudwatch-events` for AWS integration).
...
class BackgroundSession:
    ...
    async def _handle_trigger(self, event: TriggerEvent) -> None:
        ...
        try:
            ...
            # ALWAYS emit session:completed for observability
            # This is automatic - callers don't need to configure it
            ...
            # ADDITIONALLY emit custom event if configured
            # (This is additive, not a replacement for session:completed)
            ...
            # ALWAYS emit session:error for observability
            await self.event_router.emit(
                "session:error",
                {
                    "session_name": self.config.name,
            ...
            # ADDITIONALLY emit custom error event if configured
    ...
    def _create_trigger_source(
        ...
        """
        Create a trigger source from module configuration.
        
        Triggers are now loaded as modules, not hardcoded classes.
        This follows the same pattern as tools, hooks, and providers.
        """
        module_name = trigger_config.get("module")
        if not module_name:
            raise ValueError("Trigger config must specify 'module'")
        
        # Load the trigger module
        trigger_module = module_loader.load(module_name, trigger_config.get("source"))
        
        # Create and configure the trigger source
        trigger_source = trigger_module.create()
        trigger_source.configure(trigger_config.get("config", {}))
        
        # Inject event router if the trigger needs it (e.g., session-event trigger)
        if hasattr(trigger_source, "set_event_router"):
            trigger_source.set_event_router(self.event_router)
...
> **Design Principle**: Background sessions are a **bundle-level concern**, not an orchestrator concern.
> Orchestrators handle turn-by-turn execution; session lifecycle is separate.
...
session:
  orchestrator:
    ...
    config:
      # What triggers the main orchestrator (in addition to user_input)
      triggers:
        - module: trigger-session-event
          config:
...
# Background sessions are TOP-LEVEL bundle config (not nested in orchestrator)
# This separates session lifecycle from orchestration logic
background_sessions:
    ...
    triggers:
      - module: trigger-file-watcher
        config:
          patterns: ["**/*.py", "**/*.ts", "**/*.js"]
          debounce_ms: 2000
    # on_complete_emit is optional - session:completed always emits automatically
  ...
  - name: researcher
    ...
    start_on_parent_start: false  # Started explicitly via event
    triggers:
      - module: trigger-session-event
        config:
...
1. **`background_sessions` is top-level** - Not nested in orchestrator config
2. **Triggers use module syntax** - `module: trigger-file-watcher` instead of `type: file_change`
3. **Orchestrator only handles event reactions** - Not session lifecycle
4. **`session:completed` always emits** - Custom `on_complete_emit` is additive, not replacement
...
| Where do trigger implementations live? | Pluggable modules | Foundation defines protocol; implementations are swappable (feedback integration) |
| Where is background_sessions configured? | Top-level bundle config | Separates session lifecycle from orchestration logic (feedback integration) |
| How do callers get results from background sessions? | Subscribe to session:completed event | Always emitted automatically; custom events are additive (feedback integration) |
| Should SpawnResult track events_emitted? | No | Events are fire-and-forget; tracking belongs in EventRouter (feedback integration) |
| How should context summarization work? | Separate summarizer capability | Not embedded in spawn primitive; involves LLM policy decisions (feedback integration) |
...
## 12. Integrated Feedback Summary

This section documents changes made based on design review feedback.

### Adjustments Integrated

#### 1. Triggers as Pluggable Modules

**Before**: Trigger implementations (FileChangeTrigger, TimerTrigger, etc.) were hardcoded in foundation.

**After**: Foundation defines only the `TriggerSource` protocol. Implementations are loadable modules:
...
**Rationale**: Different teams may need different implementations (e.g., inotify vs watchdog, AWS CloudWatch Events vs polling). This follows "mechanism, not policy."

#### 2. Background Sessions as Top-Level Config

**Before**: `background_sessions` was nested inside `session.orchestrator.config`.

**After**: `background_sessions` is a top-level bundle configuration key, parallel to `providers`, `tools`, and `hooks`.

**Rationale**: Orchestrators handle turn-by-turn execution. Session lifecycle is a separate concern. Nesting conflated two distinct responsibilities.

#### 3. Automatic session:completed Event

**Before**: Background sessions only emitted events if `on_complete_emit` was configured.

**After**: Background sessions ALWAYS emit `session:completed` (and `session:error` on failure). Custom `on_complete_emit` is additive.

**Rationale**: Fire-and-forget spawning needs observability. Callers must be able to subscribe to completion without requiring explicit configuration.

### Minor Refinements

| Change | Rationale |
|--------|-----------|
| Added `delete()` and `list_sessions()` to SessionStorage | Common operations needed for session management |
| Removed `events_emitted` from SpawnResult | Events are fire-and-forget; tracking belongs in EventRouter history |
| Removed `ContextInheritance.SUMMARY` | Summarization involves LLM policy decisions; should be separate capability |

### Philosophy Alignment

All changes reinforce Amplifier's core principles:

- **Mechanism, not policy**: Protocol in foundation; implementations pluggable
- **Separation of concerns**: Lifecycle ≠ orchestration
- **Observability as built-in mechanism**: Always emit completion events
- **Ruthless simplicity**: Remove unnecessary coupling (events in SpawnResult, summary in spawn primitive)
